...
import { convertToPlainObject, formatError, roundTwoDecimalPlaces } from "../utils";
  ...
  try {
    ...
    if (!cart) {
      ...
      if (existItem) {
        ...
        // Check stock
        if (product.stock < 1) throw new Error("Not enough items in stock");
...
export async function removeItemFromCart(productId: string) {
  try {
    ...
    // Check for item in the user's cart
    const exist = (cart.items as CartItem[]).find((x) => x.productId === productId);
    if (!exist) throw new Error("Item not found");

    // Check quantity of item in user's cart
    if (exist.qty === 1) {
      // Quantity was 1. Remove the item from the user's cart
      cart.items = (cart.items as CartItem[]).filter((x) => x.productId !== exist.productId);
    } else {
      // Quanitity was more than 1. Decrease quanitity by 1.
